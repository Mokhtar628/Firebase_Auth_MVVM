...
class CustomCelebrationDialog extends StatefulWidget {
  ...
  const CustomCelebrationDialog({Key? key, required this.fullName}) : super(key: key);

  @override
  _CustomCelebrationDialogState createState() => _CustomCelebrationDialogState();
}

class _CustomCelebrationDialogState extends State<CustomCelebrationDialog> with TickerProviderStateMixin {
  ...
  void _generateCelebrationScraps() {
    ...
    for (int i = 0; i < 70; i++) {
      ...
      switch (imageIndex) {
        ...
        case 3:
        case 4:
          size = imageIndex == 3 ? random.nextDouble() * 7 + 15 : random.nextDouble() * 10 + 20;
          scrap = Image.asset(
            'assets/scraps/scrap$imageIndex.png',
            width: size,
            height: size,
            fit: BoxFit.contain,
          );
                ...
                child: Center(
                  child: Column(
                    children: [
                      Text(
                        "Congratulations!",
                        style: TextStyle(fontSize: 24, fontWeight: FontWeight.bold, color: AppColors.secondary, fontFamily: 'Inter'),
                      ),
                      const SizedBox(
                        height: 8,
                      ),
...
showCustomCelebrationDialog(BuildContext context, String fullName) {
  showDialog(
    ...
    builder: (_) => CustomCelebrationDialog(fullName: fullName,),
  );
}
